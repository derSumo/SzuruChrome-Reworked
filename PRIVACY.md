...
| **Scraped booru-page content** (image URL, tags, pools, source, notes) | The core purpose of the extension: turning a booru page into a szurubooru post | Read only on source sites the user enabled, either to render the optional page badge/batch UI or after an explicit import action. Transmitted only to the szurubooru instance URL the user configured. |
| **Extension settings** (selected language, tag-category colors, instance list, similarity threshold) | Persisting the user's preferences across sessions | Stored locally in `browser.storage.local`. Never transmitted. |
...
- `activeTab`, `tabs`, `scripting` — import the active page on an explicit popup, context-menu, or native keyboard-command action
...
- `optional_host_permissions` — site access is requested only when the user enables an individual supported source in Options or connects a configured szurubooru instance. There is no permanent all-sites host permission.
...
| **Gescrapter Booru-Seiten-Inhalt** (Bild-URL, Tags, Pools, Quelle, Notes) | Kernzweck der Erweiterung: Booru-Seite in szurubooru-Post umwandeln | Wird nur auf Quellseiten gelesen, die der Nutzer aktiviert hat — entweder für optionales Seiten-Badge/Batch-UI oder nach einer expliziten Importaktion. Übertragung ausschließlich an die konfigurierte szurubooru-Instanz-URL. |
| **Erweiterungs-Einstellungen** (Sprache, Tag-Kategorien-Farben, Instanz-Liste, Ähnlichkeits-Schwelle) | Persistenz der Nutzereinstellungen über Sessions hinweg | Lokal in `browser.storage.local` gespeichert. Nie übertragen. |
...
- `activeTab`, `tabs`, `scripting` — Import der aktiven Seite nach expliziter Popup-, Kontextmenü- oder nativer Tastenkürzel-Aktion
...
- `optional_host_permissions` — Seitenzugriff wird nur angefordert, wenn der Nutzer in den Optionen eine einzelne unterstützte Quelle aktiviert oder eine konfigurierte szurubooru-Instanz verbindet. Es gibt keine permanente Berechtigung für alle Websites.
