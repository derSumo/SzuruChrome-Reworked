...
import { BATCH_ID, BATCH_STYLES, SELECTABLE_CLASS, SELECTED_CLASS } from "./batchUi.styles";
...
interface BatchRun {
  ...
  /** Cancel asked for; the queue is closing but items in flight still finish. */
  cancelling?: boolean;
}
...
function describeRun(run: BatchRun): string {
  ...
  const parts = [
    run.cancelling
      ? t("batch.cancelling", { done: run.done }) || `Stopping… (${run.done} done)`
      : t("batch.progress", { done: run.done, total: run.total }) || `Importing ${run.done}/${run.total}`,
  ];
...
/**
 * Ask the background to stop this batch. Items already in flight finish — the
 * alternative is tearing a tab down mid-upload, which can leave a post half
 * created — so the row keeps ticking briefly after the click.
 */
async function requestCancelRun(batchId: string): Promise<void> {
  updateBatchRun(batchId, { cancelling: true });
  try {
    await browser.runtime.sendMessage(new BrowserCommand("batch_cancel", { batchId }));
  } catch {
    // Worker asleep or already finished; the next status message corrects us.
  }
}
  ...
  for (const run of runs) {
    ...
    row.innerHTML = `
      <div class="szb-head">
        ...
        ${!run.message && !run.cancelling ? `<button class="szb-collapse szb-cancel" type="button" title="${escapeHtml(t("batch.stopRun") || "Stop this import")}">${icon("stop", 14)}</button>` : ""}
    ...
    `;
    row.querySelector(".szb-cancel")?.addEventListener("click", () => void requestCancelRun(run.batchId));
...
export function handleBatchStatus(data: any): void {
  ...
  if (data.phase === "start" || data.phase === "progress") {
    updateBatchRun(batchId, {
      ...
      ...(data.cancelling ? { cancelling: true } : {}),
    });
    return;
  }
  ...
  if (data.cancelled) {
    let stopped = t("batch.cancelled", { ok: data.succeeded ?? 0 }) || `Stopped after ${data.succeeded ?? 0}`;
    if (data.cancelledRemaining) {
      stopped += ` · ${t("batch.cancelledRemaining", { remaining: data.cancelledRemaining }) || `${data.cancelledRemaining} not imported`}`;
    }
    if (data.poolName && data.poolId) stopped += ` · ${t("batch.pooled") || "pooled"}`;
    // Sticky: a stopped run is a decision the user made and should stay
    // readable until they close it, not vanish like a clean success.
    finishBatchRun(batchId, stopped, true);
    return;
  }
...
async function attachToRunningBatch(): Promise<void> {
  try {
    const active:
      ...
      | undefined = await browser.runtime.sendMessage(new BrowserCommand("batch_active"));
    ...
    updateBatchRun(active.batchId, {
      ...
      cancelling: active.cancelling === true,
    });
  } catch {
